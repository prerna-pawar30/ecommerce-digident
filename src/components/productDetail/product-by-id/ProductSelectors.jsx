  ...
  >
    {variants.slice(variantStartIndex, variantStartIndex + 3).map((v, i) => {
      ...
      return (
        <button
          key={v._id || actualIdx}
          onClick={() => setSelectedVariantIndex(actualIdx)}
          className={`flex-1 min-w-0 max-w-[150px] border-2 rounded-xl overflow-hidden transition-all duration-300 shadow-sm ${
            isSelected
              ? "border-[#eb730b] scale-105 shadow-md"
              : "border-orange-200 hover:border-orange-300"
          }`}
          style={{ cursor: "pointer" }}
        >
          {/* Image Container */}
          <div className="h-16 md:h-14 p-2 bg-white flex items-center justify-center">
            <img
              src={v.variantImages?.[0] || product.images?.[0]}
              className="h-full object-contain"
              alt={v.name}
            />
          </div>

          {/* Label Container with Gradient applied when selected */}
          <div
            className={`py-1 text-[14px] md:text-[12px] truncate px-1 font-bold text-center transition-colors ${
              isSelected ? "text-white" : "bg-orange-100 text-gray-600"
            }`}
            style={{
